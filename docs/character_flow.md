...
4. **Vitality** – `VitalityInfo::from_prompt()` now seeds a neutral `VitalityProfile` (control, power, cooldown efficiency) plus an empty spellbook, then records the chosen element for future trees.
...
## Vitality & Mana
- Vitality stats start low so casters/attuned heroes can grow alongside their physical counterparts. Level-ups nudge power/control while shaving cooldown %, but most bonuses will come from future training systems.
- Spell cooldowns are spell-specific; as long as mana remains, players can rotate through different spells while others recharge.
- Every character now tracks a mana archetype (`Civilian`, `Martial`, `Ranger`, `Support`, `Caster`). Switching to a caster style grants a +10 capacity buffer (cap 110) while other archetypes cap at 100.
- Mana lives directly on the `Character` record so it serializes with the rest of the profile and shows up on the overview card.
